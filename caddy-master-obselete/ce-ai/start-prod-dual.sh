...
if docker ps --format '{{.Names}}' | rg -x "${EDGE_CONTAINER_NAME}" >/dev/null 2>&1; then
  ...
  echo "[WARN] ce-ai services may start successfully but will not be publicly reachable via nip.io until edge proxy is up."
  echo "[WARN] Start edge proxy with: cd /opt/edge-proxy && docker compose up -d"
fi

echo "[INFO] Starting ce-ai production stack..."
